...
**SR-04 (HARD)** — One L3 CTA above the fold on home (hero primary = Book a scan → `/book-a-scan/`).
...
**SR-08 (HARD)** — Header CTA is always L3 Book a scan → `/book-a-scan/`. L1 proof = Proof nav → `/proof/`.
...
**SR-14 (HARD)** — Qualification paths: Wizard and/or Book a scan with expectations set — no unqualified "free call" as only path.
*Source:* conversion-pipeline §1
...
**SR-18 (HARD)** — Book a scan copy must match UI: fallback = *Request my Automation Scan slot* + payment link after fit check; checkout model only when payment + calendar are live. Forbidden main submit: `Send enquiry`.
