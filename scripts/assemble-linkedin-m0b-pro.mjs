/**
 * Assemble M0-B LinkedIn pro clip from rendered QF slide frames.
 * Usage: node scripts/assemble-linkedin-m0b-pro.mjs
 */
...
const FRAMES_DIR = path.join(root, 'docs/operations/media/m0b-slides/frames');
const OUT_DIR = path.join(root, 'docs/operations/media');
const TMP = path.join(OUT_DIR, 'm0b-tmp');

const SCENES = [
  { id: 'hook', duration: 4, kenBurns: false },
  { id: 'pivot', duration: 3, kenBurns: false },
  { id: 'intro', duration: 4, kenBurns: true },
  { id: 'foundation', duration: 4, kenBurns: true },
  { id: 'vehicle', duration: 5, kenBurns: true },
  { id: 'checkout', duration: 4, kenBurns: true },
  { id: 'bedanktChoice', duration: 5, kenBurns: true },
  { id: 'briefForm', duration: 6, kenBurns: true },
  { id: 'metrics', duration: 4, kenBurns: false },
  { id: 'cta', duration: 4, kenBurns: false },
];

const FPS = 30;
const FADE = 0.28;
const OUT = path.join(OUT_DIR, 'linkedin-m0b-wizard-quotes-pro-9x16.mp4');
...
for (const scene of SCENES) {
  ...
  if (!fs.existsSync(png)) {
    ...
    console.error('Run: node scripts/render-m0b-slides.mjs');
    process.exit(1);
  }
